...
class MovieControllerTest {
  ...
  void givenTitleKeyword_whenSearchMovie_thenReturn200() throws Exception {
    mockMvc.perform(get("/api/v1/movies")
            ...
            .andExpect(jsonPath("$.length()").isNotEmpty()); //응답 JSON 배열이 비어 있지 않은지 확인
  }


  @DisplayName("장르로 영화 검색시 해당 장르의 영화 목록 및 200 응답을 반환한다.")
  @Test
  ...
  @Test
  void givenInvalidRequet_whenCreateMovie_thenReturn400() throws Exception {
    // Given: 제목이 비어 있음
  ...
  void givenValidScreeningRequest_whenCreateScreening_thenReturn201() throws Exception {
    ...
    // When & Then
    mockMvc.perform(post("/api/v1/movies/1/screeings")
                    .contentType(MediaType.APPLICATION_JSON)
  ...
  @Test
  void givenInvalidScreeningRequest_whenCreateCscreening_thenReturn400() throws Exception {
    // Given: 상영관 정보가 유효하지 않음
    ScreeningRequestDto screeningRequestDto = new ScreeningRequestDto(0L, LocalTime.of(12, 0, 0));

    // When & Then
    mockMvc.perform(post("/api/v1/movies/1/screening")
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(objectMapper.writeValueAsString(screeningRequestDto)))
            .andExpect(status().isBadRequest());
  }
